...
| 7 | High #6 — `role: null` in `tenant.ts` + `organization-context.ts` | Pending | — |
| 8 | High #8 — LLM allowlist not enforced on agent-update paths (`agents/[id]/llm-config/route.ts`, `agents/[id]/route.ts` PATCH) | Pending | — |
| 9 | High #9 — missing decrypt in `retell/billing/route.ts` | Pending | — |
