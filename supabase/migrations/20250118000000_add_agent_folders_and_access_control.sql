...
-- 2. Add folder_id to agents table
ALTER TABLE agents
...
-- 6. RLS Policies for agent_folders
...
-- Admins can manage user-agent assignments
...
-- 8. Helper function to get accessible agents for a user
...
BEGIN
  ...
  WHERE a.tenant_id = p_tenant_id
    AND (
      -- System admin can see all agents
      EXISTS (
      ...
      -- Tenant admin/super admin can see all agents in their tenant
      OR EXISTS (
        SELECT 1 FROM user_tenants ut
        WHERE ut.user_id = p_user_id
          AND ut.tenant_id = p_tenant_id
      ...
      -- Regular users can only see agents they're assigned to
      OR EXISTS (
...
COMMENT ON FUNCTION get_accessible_agents(UUID, UUID) IS 'Returns list of agent IDs accessible to a user in a tenant based on role and assignments';
